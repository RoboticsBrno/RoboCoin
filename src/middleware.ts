...
export async function middleware(req: NextRequest) {
	...
	const isCampRoute =
		potentialCampUrl && !nonCampRoutes.includes(potentialCampUrl);
	...
	if (isCampRoute) {
		...
		if (!session) {
			...
			const isManagerPath = pathSegments.some((segment) =>
				managerPaths.includes(segment)
			);
			const isManager = session.is_manager;

			if (
				session.user_camps &&
				!(session.user_camps as string[]).includes(campUrl) &&
				isManagerPath &&
				!isManager
			) {
				const url = req.nextUrl.clone();
		...
		const isManagerPath =
			pathname.includes("add-managers") ||
			pathname.includes("create-camp") ||
			pathname === "/";
