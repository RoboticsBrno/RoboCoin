...
export default function ManageUserAchievementsPage() {
	...
	useEffect(() => {
		...
		const fetchUserInventory = async () => {
			...
			try {
				const response = await fetch(`/api/inventory/user/${selectedUserId}`);
	...
	const onFormSubmit = async (data: SyncSchema) => {
		try {
			...
			setMessage("An unexpected error occurred while updating achievements.");
			setMessageType('danger');
		}
	};
		...
		<div>
			...
			<FormProvider {...methods}>
				<FormContainer onSubmit={handleSubmit(onFormSubmit)}>
					...
					<FormSubtitle>Check or uncheck achievements to grant or revoke them.</FormSubtitle>

					<FormGroup>
						<FormSelect
							label="User"
							name="userId"
							options={users.map(user => ({ value: user.id, label: `${user.name} (${user.login})` }))}
							onChange={(e) => setSelectedUserId(e.target.value)}
						/>
					</FormGroup>

					<FormGroup>
						<label className="block text-sm font-medium text-gray-300">Achievements{isInventoryLoading ? " (loading)" : null}</label>
						{!selectedUserId ? (
							<p className="text-gray-500 text-sm mt-1">Please select a user to view their achievements.</p>
						) : (<ItemOptions items={items} currentItemIds={currentItemIds} setValue={setValue} />)}
					</FormGroup>

					<FormSubmit isLoading={isSubmitting}>Update Achievements</FormSubmit>
				</FormContainer>
			</FormProvider>
		</div>
	);
...
function ItemOptions({ items, currentItemIds, setValue }: { items: Item[], currentItemIds: number[] | undefined, setValue: UseFormSetValue<{ userId: string; itemIds: number[]; }> }) {
	return (
				...
				<FormCheckbox
					...
					onChange={(e) => {
						...
						if (checked) {
							...
							setValue('itemIds', currentIds.filter(id => id !== item.id));
						}
					}}
				/>
			))}
