...
export async function POST(req: NextRequest): Promise<NextResponse<AddManagersResponse | { error: string }>> {
	...
	const toAdd = userIds
		.filter((id: number) => !currentManagerIds.includes(id));
	const toRemove = currentManagerIds
		.filter((id: number) => !userIds.includes(id));
