...
export default function FormContainer<T extends FieldValues>({ children, onSubmit, schema }: FormContainerProps<T>) {
				...
				<div className="w-full max-w-md p-8 space-y-6 bg-gray-800 rounded-lg shadow-md">
					<form className="space-y-6" onSubmit={methods.handleSubmit(onSubmit)}>
						{React.Children.map(children, (child) => {
							if (React.isValidElement(child) && (child.type as any).displayName === 'FormSubmit') {
